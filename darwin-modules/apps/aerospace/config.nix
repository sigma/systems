...
let
  ...
  loginStart = if autostart then "true" else "false";

  switchToWorkspace =
    ws:
    if ws.display != null then
      "workspace ${ws.name}" # for lettered workspaces, bound to a monitor
    else
      "summon-workspace ${ws.name}"; # for numbered workspaces
  ...
  workspaceKeybindings = builtins.concatStringsSep "\n" (
    builtins.map (ws: ''
      ...
      ${mods.secondary}-${
        lib.toLower (builtins.substring 0 1 ws.name)
  ...
  displaySpec =
    display:
    if builtins.isString display then
      quote display
    else
      "[${builtins.concatStringsSep ", " (builtins.map quote display)}]";

  # Generate workspace-to-monitor assignments
  workspaceAssignments = concatSep "\n" (
    builtins.map (
      ws: if ws.display != null then "${ws.name} = ${displaySpec ws.display}" else ""
    ) workspaces
  );

  # config-version = 2 stops inferring persistent workspaces from the
  # keybinding RHS, so list them explicitly. Derived from every declared
  # workspace to preserve the previous (v1) behavior.
  persistentWorkspaces = "[${builtins.concatStringsSep ", " (builtins.map (ws: quote ws.name) workspaces)}]";
  ...
  generateWindowRules = builtins.concatStringsSep "\n" (
    builtins.map (rule: ''
      ...
      ${concatSep "\n" [
        ...
        (lib.optionalString (
          rule.appNameRegexSubstring != null
        ) "if.app-name-regex-substring = '${rule.appNameRegexSubstring}'")
        (lib.optionalString (
          rule.windowTitleRegexSubstring != null
        ) "if.window-title-regex-substring = '${rule.windowTitleRegexSubstring}'")
      ]}
      run = [${
        concatSep ", " [
...
''
  ...
  # Config schema version. See:
  # https://nikitabobko.github.io/AeroSpace/guide#config-version
  config-version = 2

  # Workspaces that always exist even when empty. Required under
  # config-version = 2 (v1 inferred these from the keybinding RHS).
  ...
  ${
    if borders then
      "'exec-and-forget ${bordersBinary} active_color=0xffe2e2e3 inactive_color=0xff414550 width=8.0 style=round'"
    else
      ""
  }
  ]
