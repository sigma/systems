...
(use-package! org-roam
  ...
  (setq org-id-link-to-org-use-id t
        ...
        '(("d" "default" plain "** %?" :target
           (file+head+olp "%<%G-%m>.org" "#+setupfile: ../../_setup_roam_diary.org\n#+title: %<%G-%m>\n#+filetags: :journal:\n"
                       ("[%<%Y-%m-%d %a>]"))))))
#+end_src
***** Dailies
...
;;;###autoload
(defun yrh/org-roam-dailies-add-ids ()
  (interactive)
  ...
  (let ((dailies-dir (expand-file-name org-roam-dailies-directory org-roam-directory)))
    (when (file-exists-p dailies-dir)
      (let* ((month displayed-month)
        ...
        (mapc (lambda (item)
                (let ((fname (expand-file-name (format "%04d-%02d.org" (cdr item) (car item)) dailies-dir)))
...
#+begin_src emacs-lisp :tangle modules/config/private/config.el
(add-hook 'org-roam-dailies-find-file-hook #'yrh/org-roam-dailies-add-ids)
