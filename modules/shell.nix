...
in
{
  ...
  perSystem =
    ...
    let
      ...
      preCommitSettings = config.pre-commit.settings;

      # A pre-commit hook that resolves its config from the *main* worktree.
      #
      # git-hooks.nix installs a shared hook (core.hooksPath points at the main
      # worktree's .git/hooks, so every linked `git worktree` runs it) generated
      # by `pre-commit install`. That command bakes in the config path
      # ".pre-commit-config.yaml" *relative to the toplevel* (pre-commit's
      # main._adjust_args_and_chdir always relativizes --config), so a commit
      # from a linked worktree — e.g. .claude/worktrees/* — resolves it against
      # the wrong toplevel and fails with "No .pre-commit-config.yaml file was
      # found", because linked worktrees don't carry that symlink.
      #
      # This replacement computes the main worktree from the shared git common
      # dir at *hook* time and passes an absolute --config. pre-commit abspaths
      # an existing --config before it re-relativizes, so it stays anchored to
      # the parent repo's config regardless of which worktree commits.
      preCommitHook = pkgs.writeShellScript "pre-commit-worktree-hook" ''
        # Generated by modules/shell.nix — anchors pre-commit to the main
        # worktree's config so commits from linked git worktrees work.
        HERE="$(cd "$(${pkgs.coreutils}/bin/dirname "$0")" && pwd)"
        main_wc=$(${pkgs.coreutils}/bin/dirname \
          "$(${pkgs.git}/bin/git rev-parse --path-format=absolute --git-common-dir)")
        exec ${lib.getExe preCommitSettings.package} hook-impl \
          --config="$main_wc/${preCommitSettings.configPath}" \
          --hook-type=pre-commit \
          --hook-dir "$HERE" -- "$@"
      '';

      # Runs after git-hooks.nix's own installationScript (see startup below).
      # Ensures the parent-repo config exists and swaps in the anchored hook.
      preCommitWorktreeFixup = lib.optionalString preCommitSettings.install.enable ''
        if ${pkgs.git}/bin/git rev-parse --git-dir &>/dev/null; then
          _pc_common=$(${pkgs.git}/bin/git rev-parse --path-format=absolute --git-common-dir)
          _pc_main_wc=$(${pkgs.coreutils}/bin/dirname "$_pc_common")
          _pc_cfg="$_pc_main_wc/${preCommitSettings.configPath}"

          # Ensure the config symlink exists in the main worktree (as a gc root,
          # matching git-hooks.nix), even when the devshell was only ever entered
          # from a linked worktree where the module installed it locally instead.
          if [ ! -e "$_pc_cfg" ] \
            || [ "$(readlink "$_pc_cfg" 2>/dev/null)" != "${preCommitSettings.configFile}" ]; then
            nix-store --add-root "$_pc_cfg" --indirect \
              --realise ${preCommitSettings.configFile} >/dev/null
          fi

          # Swap the shared hook for the worktree-aware one. Guarded on content
          # to avoid filesystem churn (matching git-hooks.nix's own approach).
          _pc_hook="$_pc_common/hooks/pre-commit"
          if [ ! -e "$_pc_hook" ] || ! ${pkgs.diffutils}/bin/cmp -s ${preCommitHook} "$_pc_hook"; then
            ${pkgs.coreutils}/bin/install -m755 ${preCommitHook} "$_pc_hook"
          fi
        fi
      '';
    in
